...
<br>
	...
	struct libusb_interface_descriptor {
		...
		uint8_t iInterface;
		struct const libusb_endpoint_descriptor *endpoint;
		const unsigned char *extra;
		int extra_length;
	};
	/**
...
<br>
	...
	struct libusb_interface {
		struct const libusb_interface_descriptor *altsetting;
		int num_altsetting;
	};
	/**
	 * \ingroup desc
...
<br>
	...
	struct libusb_config_descriptor {
		...
		uint8_t bmAttributes;
		uint8_t MaxPower;
		struct const libusb_interface *interface;
		const unsigned char *extra;
		int extra_length;
	};
	/**
...
<br>
	 * notified about.
	 */
	typedef void (__stdcall *libusb_transfer_cb_fn)(struct libusb_transfer* transfer);
	/**
	 * \ingroup asyncio
<br>
...
<br>
	...
	const __stdcall struct libusb_version* libusb_get_version();
	...
	const __stdcall char* libusb_strerror(enum libusb_error {
	} errcode);
	...
	__stdcall int libusb_get_device_descriptor(libusb_device* dev, struct libusb_device_descriptor* desc);
	__stdcall int libusb_get_active_config_descriptor(libusb_device* dev, struct libusb_config_descriptor** config);
	__stdcall int libusb_get_config_descriptor(libusb_device* dev, uint8_t config_index, struct libusb_config_descriptor** config);
	__stdcall int libusb_get_config_descriptor_by_value(libusb_device* dev, uint8_t bConfigurationValue, struct libusb_config_descriptor** config);
	__stdcall void libusb_free_config_descriptor(struct libusb_config_descriptor* config);
	__stdcall int libusb_get_ss_endpoint_companion_descriptor(struct libusb_context* ctx, struct const libusb_endpoint_descriptor* endpoint, struct libusb_ss_endpoint_companion_descriptor** ep_comp);
	__stdcall void libusb_free_ss_endpoint_companion_descriptor(struct libusb_ss_endpoint_companion_descriptor* ep_comp);
	__stdcall int libusb_get_bos_descriptor(libusb_device_handle* handle, struct libusb_bos_descriptor** bos);
	__stdcall void libusb_free_bos_descriptor(struct libusb_bos_descriptor* bos);
	__stdcall int libusb_get_usb_2_0_extension_descriptor(struct libusb_context* ctx, struct libusb_bos_dev_capability_descriptor* dev_cap, struct libusb_usb_2_0_extension_descriptor** usb_2_0_extension);
	__stdcall void libusb_free_usb_2_0_extension_descriptor(struct libusb_usb_2_0_extension_descriptor* usb_2_0_extension);
	__stdcall int libusb_get_ss_usb_device_capability_descriptor(struct libusb_context* ctx, struct libusb_bos_dev_capability_descriptor* dev_cap, struct libusb_ss_usb_device_capability_descriptor** ss_usb_device_cap);
	__stdcall void libusb_free_ss_usb_device_capability_descriptor(struct libusb_ss_usb_device_capability_descriptor* ss_usb_device_cap);
	__stdcall int libusb_get_container_id_descriptor(struct libusb_context* ctx, struct libusb_bos_dev_capability_descriptor* dev_cap, struct libusb_container_id_descriptor** container_id);
	__stdcall void libusb_free_container_id_descriptor(struct libusb_container_id_descriptor* container_id);
...
<br>
	...
	__stdcall struct libusb_transfer* libusb_alloc_transfer(int iso_packets);
	__stdcall int libusb_submit_transfer(struct libusb_transfer* transfer);
	__stdcall int libusb_cancel_transfer(struct libusb_transfer* transfer);
	__stdcall void libusb_free_transfer(struct libusb_transfer* transfer);
	__stdcall void libusb_transfer_set_stream_id(struct libusb_transfer* transfer, uint32_t stream_id);
	__stdcall uint32_t libusb_transfer_get_stream_id(struct libusb_transfer* transfer);
	/**
	 * \ingroup asyncio
<br>
...
<br>
	...
	static inline void libusb_fill_control_transfer(struct libusb_transfer* transfer, libusb_device_handle* dev_handle, unsigned char* buffer, libusb_transfer_cb_fn callback, void* user_data, unsigned int timeout) {
		...
		if (setup) 
			transfer->length = (int)((sizeof(struct libusb_control_setup)) + libusb_cpu_to_le16(setup->wLength));
...
<br>
	...
	__stdcall int libusb_wait_for_event(libusb_context* ctx, struct timeval* tv);
	__stdcall int libusb_handle_events_timeout(libusb_context* ctx, struct timeval* tv);
	__stdcall int libusb_handle_events_timeout_completed(libusb_context* ctx, struct timeval* tv, int* completed);
	...
	__stdcall int libusb_handle_events_locked(libusb_context* ctx, struct timeval* tv);
	__stdcall int libusb_pollfds_handle_timeouts(libusb_context* ctx);
	__stdcall int libusb_get_next_timeout(libusb_context* ctx, struct timeval* tv);
	/**
	 * \ingroup poll
<br>
...
<br>
	 ...
	 */
	typedef void (__stdcall *libusb_pollfd_added_cb)(int fd, short events, void* user_data);
	/**
	 * \ingroup poll
<br>
...
<br>
	 * \see libusb_set_pollfd_notifiers()
	 */
	typedef void (__stdcall *libusb_pollfd_removed_cb)(int fd, void* user_data);
	const __stdcall struct libusb_pollfd** libusb_get_pollfds(libusb_context* ctx);
...
<br>
	 ...
	 */
	typedef int (__stdcall *libusb_hotplug_callback_fn)(libusb_context* ctx, libusb_device* device, libusb_hotplug_event event, void* user_data);
	/**
	 * \ingroup hotplug
<br>
